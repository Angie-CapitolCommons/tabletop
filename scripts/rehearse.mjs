...
async function playRoom(room, scenarioId) {
  ...
  const log = { room, scenarioId, decisions: [], elderRounds: [], chat: [], errors: [] };
  const roster = {};
  ...
  for (let guard = 0; guard < 20; guard++) {
    ...
    const d = { decision: label, title: node.title };
    await sleep(PACE);
    ...
    for (const line of talk(3 + Math.floor(r() * 4))) await say(line);
    ...
    const heard = await elders(false, label);
    if (!heard) {
      ...
      if (vague && r() < 0.5) {
        for (const line of talk(1 + Math.floor(r() * 3))) await say(line);
  ...
  for (const p of parts.slice(0, 2)) await say(`If we'd named someone at ${p.type}, month ${p.month} might have gone the other way.`, { focus: p.nodeId });
  for (const q of QUESTIONS.slice(0, CHAT)) {
    try {
